...
MODULE_DESCRIPTION("Kernel-level process memory reader with page decryption monitor, syscall tracing, decrypt hook, write, module enum, pattern scan, and poll/epoll");
MODULE_VERSION("2.7");
...
struct active_uprobe {
    int active;
    int pid;
    u64 start_time;                 // Task start_boottime — distinguishes PID reuse (audit #21)
...
// ============================================================================
// Async Event Notification State (poll/epoll)
// ============================================================================

// Wait queue used to wake userspace when any subsystem produces events.
// Producer sites (bp_handler, syscall_trace_callback, uprobe_handler,
// decrypt_hook_handler, capture_all_threads) call wake_up_interruptible()
// on this queue *after* releasing their respective subsystem lock.
static DECLARE_WAIT_QUEUE_HEAD(memreader_event_wq);

// ============================================================================
// HW Breakpoint Auto-Cleanup on Thread Exit (audit #3/#8 full fix)
// ============================================================================
//
// HW breakpoints are registered per-thread via register_user_hw_breakpoint().
// When a target thread exits, the kernel's perf_event core deactivates the
// event but our reference keeps the perf_event allocation alive. To free it
// promptly we hook the sched_process_exit tracepoint, look up the exiting
// thread's tid in our tracking arrays, atomically take ownership of the
// perf_event pointer, and queue a workqueue item to do the actual
// unregister_hw_breakpoint() — that call may sleep, but the tracepoint
// callback runs in atomic context, so we cannot sleep there.

static struct workqueue_struct *bp_cleanup_wq = NULL;
static struct tracepoint *tp_sched_process_exit = NULL;
static bool sched_exit_probe_registered = false;

struct bp_cleanup_work {
    struct work_struct work;
    struct perf_event *bp_event;
};

static void bp_cleanup_worker(struct work_struct *w) {
    struct bp_cleanup_work *job = container_of(w, struct bp_cleanup_work, work);
    if (job->bp_event) {
        unregister_hw_breakpoint(job->bp_event);
    }
    kfree(job);
}

// Queue a perf_event for deferred unregistration. Caller has already detached
// the pointer from any tracking array under the appropriate lock.
static void queue_bp_cleanup(struct perf_event *bp) {
    struct bp_cleanup_work *job;
    if (!bp || !bp_cleanup_wq) {
        // No workqueue available (init failed) — leak rather than crash.
        return;
    }
    job = kmalloc(sizeof(*job), GFP_ATOMIC);
    if (!job) {
        // Allocation failure in atomic context: leak the event rather than
        // sleeping or calling unregister from atomic context.
        return;
    }
    INIT_WORK(&job->work, bp_cleanup_worker);
    job->bp_event = bp;
    queue_work(bp_cleanup_wq, &job->work);
}
                       ...
                       struct pt_regs *regs)
{
    ...
    if (bp_num >= 0)
        wake_up_interruptible(&memreader_event_wq);
}
...
static void capture_all_threads(int pid, unsigned long page_addr) {
    ...
    // Audit #2: take a reference on each thread before reading its pt_regs.
    // for_each_thread under rcu_read_lock keeps the task_struct allocation
    // alive, but task_pt_regs(thread) reads into thread->stack which races
    // with thread exit. We grab a ref under RCU, drop RCU, read regs, then
    // put the ref. PF_EXITING threads are skipped to avoid touching a stack
    // that's mid-teardown.
    rcu_read_lock();
    for_each_thread(task, thread) {
        ...
        // Equivalent of get_task_struct_rcu(): bump refcount only if non-zero,
        // so we don't resurrect a task that's already dropped to zero.
        if (!refcount_inc_not_zero(&thread->usage)) continue;
        rcu_read_unlock();

        regs = task_pt_regs(thread);
        if (regs && !(thread->flags & PF_EXITING)) {
...
static long start_page_monitor(struct memreader_page_monitor_config __user *user_config) {
    ...
    // Audit #14 corollary: tighten busy check — if a previous stop is still
    // tearing down (active==0 but thread handle not yet cleared), reject.
    ...
    // Audit #14: don't hold monitor_mutex across kthread_run. The new thread
    // could try to take it (via capture_all_threads) and we want to keep
    // the lock-hold time short. Mark active=1 first, drop the mutex, then
    // start the thread. If kthread_run fails, reacquire to roll back.
...
// Audit #1/#20: kthread_stop() must NOT be called with monitor_mutex held —
// the monitor thread calls capture_all_threads() which takes the same mutex,
// so holding it here would deadlock. We snapshot the thread handle, drop the
// mutex, kthread_stop, then reacquire to free buffers. start_page_monitor()
// rejects starts while monitor_thread is non-NULL, so the start/stop race
// window is closed by ordering: we clear monitor_thread last.
static long stop_page_monitor(void) {
    struct task_struct *thread_to_stop = NULL;

    mutex_lock(&monitor_mutex);
    if (monitor_state.active && monitor_state.monitor_thread) {
        monitor_state.active = 0;        // Signal thread to exit on next loop check
        thread_to_stop = monitor_state.monitor_thread;
    }
    mutex_unlock(&monitor_mutex);

    // Wait for thread to exit WITHOUT holding the mutex
    if (thread_to_stop) {
        kthread_stop(thread_to_stop);
    }

    // Thread is gone; safe to free shared buffers
    mutex_lock(&monitor_mutex);
    monitor_state.monitor_thread = NULL;
    if (monitor_state.pages) {
        kvfree(monitor_state.pages);
...
static long get_decrypt_capture(struct memreader_decrypt_capture __user *user_capture) {
    ...
    // Audit #13: copy captures FIRST, then header. If captures copy fails,
    // the header is never written, leaving userspace's view consistent
    // (its existing buffer is untouched and it gets -EFAULT). The previous
    // ordering could leave userspace seeing a header advertising N entries
    // when 0 had actually been copied.
    if (captures_buf && header.captures) {
        ...
        kvfree(captures_buf);
    } else if (captures_buf) {
        // Allocated but no userspace target — defensive free
        kvfree(captures_buf);
    }
...
static long start_syscall_trace(struct memreader_syscall_config __user *user_config) {
    ...
    // Allocate buffer if not already. We can't allocate under a spinlock,
    // so drop -> alloc -> reacquire and re-check both `active` and `buffer`
    // (another caller racing may have set them in between). Fix for audit #5.
    if (!syscall_state.buffer) {
        struct memreader_syscall_entry *new_buf;

        spin_unlock_irqrestore(&syscall_lock, flags);
        new_buf = kvzalloc(sizeof(struct memreader_syscall_entry) * MAX_SYSCALL_TRACES,
                           GFP_KERNEL);
        if (!new_buf)
            return -ENOMEM;
        ...
        if (syscall_state.buffer) {
            // Another caller allocated first; discard ours
            spin_unlock_irqrestore(&syscall_lock, flags);
            kvfree(new_buf);
            spin_lock_irqsave(&syscall_lock, flags);
        } else {
            syscall_state.buffer = new_buf;
        }
    }
...
static long get_syscall_traces(struct memreader_syscall_buffer __user *user_buf) {
    ...
    // Cap requested size — prevents userspace from coercing a huge kvzalloc
    if (buf.max_entries > MAX_SYSCALL_TRACES)
        buf.max_entries = MAX_SYSCALL_TRACES;
    ...
    // Defensive double-cap: count can never legitimately exceed the ring size
    to_copy = min(buf.max_entries, syscall_state.count);
    if (to_copy > MAX_SYSCALL_TRACES)
        to_copy = MAX_SYSCALL_TRACES;
...
static int uprobe_handler(struct uprobe_consumer *uc, struct pt_regs *regs, __u64 *data)
{
    ...
    // Filter by PID (tgid). Audit #21: also check start_boottime so a recycled
    // PID belonging to a different process cannot generate hits intended for
    // the original target.
    if (up->pid != 0) {
        struct task_struct *grp = current->group_leader;
        if (current->tgid != up->pid)
            goto skip;
        if (up->start_time && grp && grp->start_boottime != up->start_time)
            goto skip;
        goto pass;
skip:
        spin_unlock_irqrestore(&uprobe_lock, flags);
                               ...
                               struct inode **out_inode, unsigned long *out_offset)
{
    ...
    // Audit #10: validate offset is inside the file. uprobe_register on a
    // bogus offset can confuse the uprobe subsystem; reject early.
    if ((loff_t)offset >= i_size_read(inode)) {
        printk(KERN_WARNING "MemReader: file offset 0x%lx beyond inode size 0x%llx\n",
               offset, (unsigned long long)i_size_read(inode));
        ret = -EINVAL;
        goto out;
    }
...
static long set_uprobe(struct memreader_uprobe_request *req)
{
    ...
    // Audit #21: snapshot the target's start_boottime so the handler can
    // distinguish PID reuse — if our PID is later recycled by an unrelated
    // process, its start_time will differ and we won't fire stale events.
    probe_task = get_task_by_pid(req->pid);
    if (probe_task) {
        probe_start_time = probe_task->start_boottime;
        put_task_struct(probe_task);
    }
    ...
    up->active = 1;
    up->pid = req->pid;
    up->start_time = probe_start_time;
...
static long set_decrypt_hook(struct memreader_decrypt_hook_config *config)
{
    ...
    // Atomically claim the hook (audit follow-up to #3/#8): the original code
    // dropped the lock between checking and setting `active`, so two concurrent
    // set_decrypt_hook callers could both pass the busy check and race on
    // bp_events[]. Mark active=1 inside the same critical section as the check.
    spin_lock_irqsave(&decrypt_hook_lock, flags);
    if (decrypt_hook.active) {
        spin_unlock_irqrestore(&decrypt_hook_lock, flags);
        return -EBUSY;
    ...
    // Store state. If clear_decrypt_hook ran concurrently (set active=0)
    // we must NOT re-enable the hook — the entries buffer was freed and
    // any subsequent handler firing would UAF. Unregister our newly-made
    // breakpoints and bail.
    spin_lock_irqsave(&decrypt_hook_lock, flags);
    if (!decrypt_hook.active) {
        spin_unlock_irqrestore(&decrypt_hook_lock, flags);
        for (i = 0; i < num_bps; i++) {
...
static long update_decrypt_hook(void)
{
    ...
    for_each_thread(leader, thread) {
        ...
        if (!IS_ERR(bp_event)) {
            spin_lock_irqsave(&decrypt_hook_lock, flags);
            // Audit #9: another caller of update_decrypt_hook / set_decrypt_hook
            // may have incremented num_breakpoints since our earlier capacity
            // check. Recheck under the lock; if full now, unregister and drop.
            // Audit #3/#8: also recheck `active` — clear_decrypt_hook may have
            // run concurrently. If we missed it, our newly-registered event
            // would leak (clear_decrypt_hook already snapshotted num_bps).
            if (!decrypt_hook.active ||
                decrypt_hook.num_breakpoints >= MAX_THREAD_BREAKPOINTS) {
                spin_unlock_irqrestore(&decrypt_hook_lock, flags);
                unregister_hw_breakpoint(bp_event);
            } else {
...
// ============================================================================
// sched_process_exit tracepoint callback — auto-cleanup HW breakpoints
// (audit #3/#8 full fix)
// ============================================================================

// Prune any active_breakpoint slots whose tracked task matches the dying tid.
// Returns number of perf_events queued for unregistration. Iterates one
// match at a time to keep stack usage bounded — MAX_BREAKPOINTS is small (4)
// so the loop runs at most a handful of times.
static int prune_breakpoints_for_pid(pid_t exiting_pid) {
    unsigned long flags;
    int i, queued = 0;

    while (1) {
        struct perf_event *bp_to_free = NULL;

        spin_lock_irqsave(&bp_lock, flags);
        for (i = 0; i < MAX_BREAKPOINTS; i++) {
            if (breakpoints[i].active &&
                breakpoints[i].pid == exiting_pid &&
                breakpoints[i].bp_event) {
                bp_to_free = breakpoints[i].bp_event;
        ...
        if (!bp_to_free)
            break;
        queue_bp_cleanup(bp_to_free);
        queued++;
    }
    return queued;
}

// Prune any decrypt_hook.bp_events[] entries whose tid matches the dying tid.
// Compacts the array; returns number of events queued for unregistration.
// TIDs are unique kernel-wide so at most one slot can match in a normal run,
// but we still scan defensively in case the array somehow has duplicates.
static int prune_decrypt_hook_for_pid(pid_t exiting_pid) {
    unsigned long flags;
    int i, queued = 0;
    ...
    while (1) {
        ...
        for (i = 0; i < decrypt_hook.num_breakpoints; i++) {
            if (decrypt_hook.bp_tids[i] == exiting_pid) {
                int last = decrypt_hook.num_breakpoints - 1;
                bp_to_free = decrypt_hook.bp_events[i];
                if (i != last) {
                    decrypt_hook.bp_events[i] = decrypt_hook.bp_events[last];
                    decrypt_hook.bp_tids[i]   = decrypt_hook.bp_tids[last];
                }
                decrypt_hook.bp_events[last] = NULL;
                decrypt_hook.bp_tids[last]   = 0;
                decrypt_hook.num_breakpoints--;
                break;
            }
        }
...
// Tracepoint signature: void (*)(void *data, struct task_struct *p)
static void sched_exit_callback(void *data, struct task_struct *p) {
    if (!p)
        return;
    // p->pid here is the TID (kernel-internal). Match against bp_tids /
    // breakpoints[].pid which both store thread identifiers.
    prune_breakpoints_for_pid(p->pid);
    prune_decrypt_hook_for_pid(p->pid);
}

// ============================================================================
// Memory Write (mirror of read path with FOLL_WRITE | FOLL_FORCE)
// ============================================================================

static long write_process_memory(int pid, unsigned long addr,
                                 const void __user *user_buffer, unsigned long size) {
    struct task_struct *task;
    struct mm_struct *mm;
    void *kernel_buffer;
    int bytes_written;
    ...
    bytes_written = access_process_vm(task, addr, kernel_buffer, size,
                                      FOLL_WRITE | FOLL_FORCE);

    if (bytes_written <= 0) {
        printk_ratelimited(KERN_DEBUG "MemReader: write failed pid=%d addr=0x%lx size=%lu\n",
                           pid, addr, size);
        ret = -EFAULT;
    } else if ((unsigned long)bytes_written < size) {
        printk_ratelimited(KERN_DEBUG "MemReader: partial write pid=%d addr=0x%lx %d/%lu\n",
                           pid, addr, bytes_written, size);
        ret = bytes_written;
    } else {
        ret = bytes_written;
    }
...
// ============================================================================
// Module Enumeration (file-backed VMAs grouped by file)
// ============================================================================

#define MEMREADER_MAX_MODULES_CAP 4096

static long list_process_modules(struct memreader_module_list *list) {
    struct task_struct *task;
    struct mm_struct *mm;
    struct vm_area_struct *vma;
    struct memreader_module_entry *kbuf = NULL;
    char *pathbuf = NULL;
    int max_modules;
    int count = 0;
    long ret = 0;
    ...
    if (list->pid <= 0)
        return -EINVAL;
    max_modules = list->max_modules;
    if (max_modules < 0)
        return -EINVAL;
    if (max_modules > MEMREADER_MAX_MODULES_CAP)
        max_modules = MEMREADER_MAX_MODULES_CAP;
    ...
    if (max_modules > 0) {
        kbuf = kvzalloc(max_modules * sizeof(*kbuf), GFP_KERNEL);
        if (!kbuf) {
            mmput(mm);
            put_task_struct(task);
            return -ENOMEM;
    ...
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
            ...
            path = d_path(&vma->vm_file->f_path, pathbuf, PATH_MAX);
            if (IS_ERR(path))
                continue;

            base_name = kbasename(path);

            prot = 0;
            if (vma->vm_flags & VM_READ)  prot |= MEMREADER_PROT_R;
            if (vma->vm_flags & VM_WRITE) prot |= MEMREADER_PROT_W;
            if (vma->vm_flags & VM_EXEC)  prot |= MEMREADER_PROT_X;

            for (i = 0; i < count && i < max_modules; i++) {
                if (strncmp(kbuf[i].name, base_name, MAX_MODULE_NAME) == 0) {
                    found = i;
                    break;
                }
            }
            ...
            if (found >= 0) {
                if (vma->vm_start < kbuf[found].base)
                    kbuf[found].base = vma->vm_start;
                if (vma->vm_end > kbuf[found].end)
                    kbuf[found].end = vma->vm_end;
                kbuf[found].size = kbuf[found].end - kbuf[found].base;
                kbuf[found].prot |= prot;
            } else {
                if (count < max_modules) {
                    kbuf[count].base = vma->vm_start;
                    kbuf[count].end  = vma->vm_end;
                    kbuf[count].size = vma->vm_end - vma->vm_start;
                    kbuf[count].prot = prot;
                    strscpy(kbuf[count].name, base_name, MAX_MODULE_NAME);
                }
                count++;
            }
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
    ...
    list->num_modules = count;

    if (count > 0 && max_modules > 0 && list->modules) {
        int to_copy = min(count, max_modules);
        if (copy_to_user(list->modules, kbuf,
                         to_copy * sizeof(struct memreader_module_entry))) {
            ret = -EFAULT;
        }
    }
...
// ============================================================================
// Pattern Scanning (in-kernel masked byte scan with chunked overlap)
// ============================================================================

#define PATTERN_SCAN_CHUNK_SIZE (64 * 1024)

static bool memcmp_masked(const unsigned char *hay,
...
static long pattern_scan_process(struct memreader_pattern_scan *req) {
    struct task_struct *task;
    struct mm_struct *mm;
    struct vm_area_struct *vma;
    ...
    if (req->pid <= 0)
        return -EINVAL;
    if (req->pattern_len < 1 || req->pattern_len > MAX_PATTERN_LEN)
        return -EINVAL;
    if (req->max_results <= 0 || req->max_results > MAX_PATTERN_RESULTS)
        return -EINVAL;
    if (!req->results)
        return -EFAULT;
    ...
    chunk_buf = kvmalloc(PATTERN_SCAN_CHUNK_SIZE, GFP_KERNEL);
    if (!chunk_buf) {
        ret = -ENOMEM;
        goto out_mm;
    }

    results = kvmalloc(req->max_results * sizeof(unsigned long), GFP_KERNEL);
    if (!results) {
        ret = -ENOMEM;
        goto out_chunk;
    }
    ...
    // end==0 means "scan everything"; ULONG_MAX is fine — the VMA iteration
    // bounds the actual range, and we already require pattern_len <= region.
    scan_end = req->end ? req->end : ULONG_MAX;
    if (scan_end <= req->start) {
        mmap_read_unlock(mm);
        goto out_copy;
    }
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0)
    {
        VMA_ITERATOR(vmi, mm, req->start);
        for_each_vma(vmi, vma) {
#else
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
        if (vma->vm_end <= req->start)
            continue;
#endif
            ...
            vma_start = max(vma->vm_start, req->start);
            vma_end = min(vma->vm_end, scan_end);
            if (vma_end <= vma_start)
                continue;
            if (vma_end - vma_start < (unsigned long)req->pattern_len)
                continue;

            cursor = vma_start;
            while (cursor + req->pattern_len <= vma_end) {
                unsigned long remaining = vma_end - cursor;
                unsigned long want = min((unsigned long)PATTERN_SCAN_CHUNK_SIZE, remaining);
                int got;
                unsigned long search_limit;
                unsigned long i;

                got = access_process_vm(task, cursor, chunk_buf, want, 0);
                if (got <= 0)
                    break;
                if ((unsigned long)got < (unsigned long)req->pattern_len)
                    break;

                search_limit = (unsigned long)got - (unsigned long)req->pattern_len + 1;
                for (i = 0; i < search_limit; i++) {
                    if (memcmp_masked(chunk_buf + i, req->pattern, req->mask,
                                      req->pattern_len)) {
                        if (stored_matches < req->max_results)
                            results[stored_matches++] = cursor + i;
                        total_matches++;
                    }
                }
                ...
                if ((unsigned long)got < want)
                    break;
                cursor += (unsigned long)got - (unsigned long)(req->pattern_len - 1);
...
out_copy:
    req->num_results = total_matches;
    if (stored_matches > 0) {
        if (copy_to_user((void __user *)req->results, results,
                         stored_matches * sizeof(unsigned long)))
            ret = -EFAULT;
    }
...
// ============================================================================
// Async Event Notification (poll/epoll + GET_EVENT_MASK ioctl)
// ============================================================================

static unsigned int memreader_pending_mask(void) {
    unsigned int mask = 0;
    unsigned long flags;
    int i;
    ...
    for (i = 0; i < MAX_HIT_RECORDS; i++) {
        if (hit_records[i].valid) {
            mask |= MEMREADER_EVENT_BP_HIT;
            break;
        }
    }
    spin_unlock_irqrestore(&bp_lock, flags);

    // monitor_mutex protects monitor_state.capture_count; trylock to keep
    // poll() non-blocking. If contended we just skip — next wake_up retries.
    if (mutex_trylock(&monitor_mutex)) {
        if (monitor_state.capture_count > 0)
            mask |= MEMREADER_EVENT_DECRYPT_CAPTURE;
        mutex_unlock(&monitor_mutex);
    }

    spin_lock_irqsave(&syscall_lock, flags);
    if (syscall_state.count > 0)
        mask |= MEMREADER_EVENT_SYSCALL;
    spin_unlock_irqrestore(&syscall_lock, flags);

    spin_lock_irqsave(&uprobe_lock, flags);
    for (i = 0; i < MAX_UPROBES; i++) {
        if (uprobes[i].active && uprobes[i].count > 0) {
            mask |= MEMREADER_EVENT_UPROBE;
            break;
        }
    }
    spin_unlock_irqrestore(&uprobe_lock, flags);
    ...
    spin_lock_irqsave(&decrypt_hook_lock, flags);
    if (decrypt_hook.count > 0)
        mask |= MEMREADER_EVENT_DECRYPT_HOOK;
    spin_unlock_irqrestore(&decrypt_hook_lock, flags);

    return mask;
}

static __poll_t memreader_poll(struct file *file, struct poll_table_struct *wait) {
    __poll_t mask = 0;

    poll_wait(file, &memreader_event_wq, wait);

    if (memreader_pending_mask() != 0)
        mask |= POLLIN | POLLRDNORM;

    return mask;
}

static long get_event_mask(unsigned int __user *user_mask) {
    unsigned int mask = memreader_pending_mask();
...
static long memreader_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case MEMREADER_WRITE_MEMORY: {
            struct memreader_write_request req;
            if (copy_from_user(&req, (void __user *)arg, sizeof(req))) {
                return -EFAULT;
            }
            ret = write_process_memory(req.pid, req.address, req.buffer, req.size);
            break;
        }

        case MEMREADER_LIST_MODULES: {
            struct memreader_module_list list;
            if (copy_from_user(&list, (void __user *)arg, sizeof(list))) {
                return -EFAULT;
            }
            ret = list_process_modules(&list);
            if (ret == 0) {
        ...
        case MEMREADER_PATTERN_SCAN: {
            struct memreader_pattern_scan req;
            if (copy_from_user(&req, (void __user *)arg, sizeof(req))) {
                return -EFAULT;
            }
            ret = pattern_scan_process(&req);
            if (ret == 0) {
                if (copy_to_user((void __user *)arg, &req, sizeof(req))) {
                    return -EFAULT;
                }
        ...
        case MEMREADER_GET_EVENT_MASK:
            ret = get_event_mask((unsigned int __user *)arg);
            break;
...
// ============================================================================
// File open/release: track open count, tear down active operations on last
// close so an in-flight ioctl never writes to a userspace buffer that has
// already been unmapped (audit #18).
// ============================================================================

static atomic_t memreader_open_count = ATOMIC_INIT(0);
...
static int memreader_release(struct inode *inode, struct file *file)
{
    ...
    // Last close: stop all asynchronous producers that could otherwise
    // still touch userspace buffers (or their own ring buffers) after
    // this fd is gone. Each helper is internally idempotent / no-op if
    // the subsystem isn't active.
    stop_syscall_trace();
    stop_page_monitor();
    clear_decrypt_hook();

    // Tear down any uprobes still active.
    for (i = 0; i < MAX_UPROBES; i++) {
        if (uprobes[i].active) {
            struct memreader_uprobe_request req = { .probe_id = i };
            clear_uprobe(&req);
        }
    }

    // Wake any thread blocked in poll() so it sees fd closure / EOF semantics.
    wake_up_interruptible(&memreader_event_wq);
    return 0;
}
...
static int __init memreader_init(void) {
    ...
    printk(KERN_INFO "MemReader: Initializing module v2.7 (full audit fixes + sched_exit cleanup)\n");
    ...
    // Audit #3/#8 full fix: workqueue + sched_process_exit tracepoint for
    // automatic HW breakpoint cleanup when target threads exit. Failures
    // here are non-fatal — we degrade to "cleanup happens at clear/unload"
    // (the previous behavior), which is leak-only, never crashy.
    bp_cleanup_wq = alloc_workqueue("memreader_bp_wq", WQ_UNBOUND, 0);
    if (!bp_cleanup_wq) {
        printk(KERN_WARNING "MemReader: failed to alloc bp cleanup workqueue; "
                            "auto-cleanup disabled\n");
    } else {
        unsigned long tp_addr = lookup_symbol("__tracepoint_sched_process_exit");
        if (tp_addr) {
            tp_sched_process_exit = (struct tracepoint *)tp_addr;
            if (tracepoint_probe_register(tp_sched_process_exit,
                                          sched_exit_callback, NULL) == 0) {
                sched_exit_probe_registered = true;
                printk(KERN_INFO "MemReader: sched_process_exit auto-cleanup armed\n");
            } else {
                printk(KERN_WARNING "MemReader: failed to register "
                                    "sched_process_exit probe\n");
                tp_sched_process_exit = NULL;
            }
        } else {
            printk(KERN_WARNING "MemReader: __tracepoint_sched_process_exit "
                                "not found; auto-cleanup disabled\n");
        }
    }
...
static void __exit memreader_exit(void) {
    ...
    // Stop syscall tracing if active. Audit #7: even if active==0 we want
    // to ensure no in-flight callback can still touch syscall_state.buffer
    // before we free it. If active==1, unregister+sync. Then drain any
    // residual callbacks via synchronize_rcu (tracepoints use RCU).
    if (syscall_state.active) {
        syscall_state.active = 0;
        if (tp_sys_enter) {
            tracepoint_probe_unregister(tp_sys_enter, syscall_trace_callback, NULL);
    ...
    // Belt-and-suspenders: tracepoint_synchronize_unregister already drains,
    // but a stray RCU reader on a multi-core system is cheap to wait for.
    synchronize_rcu();

    // Free syscall trace buffer (now safe — no callback can be in flight)
    if (syscall_state.buffer) {
    ...
    // Audit #3/#8 teardown: stop the auto-cleanup tracepoint BEFORE we touch
    // breakpoints[], so the callback can't race with our final unregister.
    if (sched_exit_probe_registered && tp_sched_process_exit) {
        tracepoint_probe_unregister(tp_sched_process_exit, sched_exit_callback, NULL);
        tracepoint_synchronize_unregister();
        sched_exit_probe_registered = false;
    }
    // Drain any deferred bp unregister work that the callback may have queued.
    if (bp_cleanup_wq) {
        flush_workqueue(bp_cleanup_wq);
    }
    ...
    // Destroy workqueue last (after all unregister calls have run)
    if (bp_cleanup_wq) {
        destroy_workqueue(bp_cleanup_wq);
        bp_cleanup_wq = NULL;
    }
