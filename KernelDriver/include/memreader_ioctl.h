...
// Structure for writing memory
struct memreader_write_request {
    int pid;                    // Target process ID
    unsigned long address;      // Memory address to write
    unsigned long size;         // Number of bytes to write
    const void *buffer;         // User-space buffer with data to write
};
...
// Permission bits for memreader_module_entry.prot
#define MEMREADER_PROT_R    (1u << 0)
#define MEMREADER_PROT_W    (1u << 1)
#define MEMREADER_PROT_X    (1u << 2)

struct memreader_module_entry {
    unsigned long base;             // Lowest vm_start across all VMAs of this file
    unsigned long size;             // end - base
    unsigned long end;              // Highest vm_end across all VMAs of this file
    unsigned int prot;              // OR of MEMREADER_PROT_* across all VMAs
    char name[MAX_MODULE_NAME];     // Basename of file path (NUL-terminated)
};

struct memreader_module_list {
    int pid;                        // Target process ID (input)
    int max_modules;                // Max entries in modules array (input)
    int num_modules;                // Output: true count (may exceed max_modules)
    struct memreader_module_entry *modules;  // Userspace ptr to entry array
};
...
#define MAX_PATTERN_RESULTS 4096

struct memreader_pattern_scan {
    int pid;                                    // Target process ID
    unsigned long start;                        // Scan range begin (inclusive)
    unsigned long end;                          // Scan range end (exclusive); 0 = whole AS
    unsigned char pattern[MAX_PATTERN_LEN];     // Bytes to match
    unsigned char mask[MAX_PATTERN_LEN];        // 0xFF = match exactly, 0x00 = wildcard
    int pattern_len;                            // 1..MAX_PATTERN_LEN
    int max_results;                            // Input cap (1..MAX_PATTERN_RESULTS)
    int num_results;                            // Output: true count (even if truncated)
    unsigned long *results;                     // Userspace u64 array of match addrs
};
...
#define MEMREADER_WRITE_MEMORY  _IOW(MEMREADER_MAGIC, 3, struct memreader_write_request)
#define MEMREADER_LIST_MODULES  _IOWR(MEMREADER_MAGIC, 4, struct memreader_module_list)
#define MEMREADER_PATTERN_SCAN  _IOWR(MEMREADER_MAGIC, 5, struct memreader_pattern_scan)
#define MEMREADER_GET_EVENT_MASK _IOR(MEMREADER_MAGIC, 6, unsigned int)

// Async event notification bits (returned by GET_EVENT_MASK)
#define MEMREADER_EVENT_BP_HIT          0x01
#define MEMREADER_EVENT_DECRYPT_CAPTURE 0x02
#define MEMREADER_EVENT_SYSCALL         0x04
#define MEMREADER_EVENT_UPROBE          0x08
#define MEMREADER_EVENT_DECRYPT_HOOK    0x10
