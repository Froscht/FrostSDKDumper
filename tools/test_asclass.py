...
Addr = 0x157580d00
print(f"=== ASClass SocialPlayerViewModel_1 @ 0x{Addr:X} ===")
...
for Off in range(0x60, 0x200, 8):
    ...
    OwnerStr = f"MATCH" if OwnerClean == Addr else f"0x{OwnerClean:X}"
    print(f"  +0x{Off:03X}: head=0x{Head:X} owner={OwnerStr} chain={ChainLen} valid={ValidCount} zero={ZeroCount}")

print(f"\nDumping first chain in detail:")
for Off in range(0x60, 0x200, 8):
    Head = ReadU64(Addr + Off)
    if not IsValidPtr(Head):
        continue
    ...
    print(f"\n  Chain from +0x{Off:03X}:")
    F = Head
    Depth = 0
    while IsValidPtr(F) and Depth < 20:
        Enc = ReadU64(F + FFIELD_NAMEPRIVATE)
        Owner = ReadU64(F + FFIELD_OWNER)
        OwnerClean = Owner & ~1
        Ci = V616Ci(Enc) if Enc else 0
        Status = "OK" if (Enc and 1 < Ci < 0x6A00000) else ("ZERO" if Enc == 0 else f"BAD({Ci})")
        print(f"    [{Depth}] ff=0x{F:X} enc=0x{Enc:016X} ci={Ci} owner=0x{OwnerClean:X} → {Status}")
        F = ReadU64(F + FFIELD_NEXT)
        Depth += 1
    break
