...
// Generated by FrostDumper on Aug 11 2026 22:50:39
// Patch image size: 0x117E9000
// =============================================================================
//
...
constexpr uint64_t GNAME_POOL_BASE    = 0xE2CE7F4ULL;  // MODULE_BASE + this
constexpr uint64_t KEY_TABLE_RVA      = 0xE2CE7F4ULL;  // MODULE_BASE + this
constexpr uint64_t GWORLD_RVA         = 0xE7F09F8ULL;  // MODULE_BASE + this → UWorld**
constexpr uint64_t GOBJECT_ARRAY_RVA  = 0xE64B1D0ULL;  // MODULE_BASE + this → FChunkedFixedUObjectArray

// ─── 1. UObject Slot Decode ──────────────────────────────────────────────────
// Pipeline: ROL64(7) → PSHUFLW(0x4B) → ROL32(17) → extract lo64
...
static inline uint64_t DecryptUObjectSlot(const uint8_t Enc[16]) {
    ...
    Lo = Rotl64(Lo, 7);
    Hi = Rotl64(Hi, 7);
    Lo = SoftPshuflw(Lo, 0x4B);
    uint32_t D0 = Rotl32((uint32_t)Lo, 17);
    uint32_t D1 = Rotl32((uint32_t)(Lo >> 32), 17);
...
static inline uint64_t DecryptUObjectSlot_QD(const uint8_t Enc[16]) {
    ...
    memcpy(&Lo, Enc, 8);
    memcpy(&Hi, Enc + 8, 8);
    { uint8_t M[16]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
static inline int ComputeSlotIndex(uint64_t ObjAddr) {
    ...
    uint32_t H = Rotl32(Lo, 25) * 0x01000193u + 0x20193F54u;
    H = Rotl32(H, 15) * 0x01000193u + Hi + 0x20193F54u;
    H = Rotl32(H, 25) * 0x01000193u + 0x20193F54u;
    H = Rotl32(H, 15) * 0x01000193u + 0x20193F54u;
    return ((H ^ (H >> 16)) & 3) ^ 2;
}
...
static inline int32_t DecryptPropertyOffset(uint32_t Stored) {
    return (int32_t)(Bswap32(Stored) ^ 0xEE0CA1CBu);
}
...
// FNV64 fold: ROL(40) + ADD, ROL(41) + ADD
...
static inline uint64_t FNameResolveEntry(uint32_t CI) {
    ...
    // Shard hash
    uint64_t SeedAddr = ChunkAddr + 0x6550ULL;
    uint32_t SeedLo = 0, SeedHi = 0;
    READ_MEMORY(SeedAddr, &SeedLo, 4);
    READ_MEMORY(SeedAddr + 4, &SeedHi, 4);
    uint32_t H = 0x01000193u * Rotl32(SeedLo, 21) + 0x6E149835u;
    H = 0x01000193u * Rotl32(H, 26) + SeedHi + 0x6E149835u;
    H = 0x01000193u * Rotl32(H, 21) + 0x6E149835u;
    uint32_t T = Rotl32(H, 11);
    uint8_t Pa = (uint8_t)(-109 * T + 82);
    uint8_t Pb = (uint8_t)((0x01000193u * T + 0x6E149835u) >> 16);
    int Bidx1 = (Pa ^ Pb) & 7;
    int Bidx2 = (Bidx1 + 1) & 7;

    // Block decode
    uint64_t BlockBase = ChunkAddr + 0x6560ULL;
    uint64_t Raw1 = 0, Raw2 = 0;
    READ_MEMORY(BlockBase + Bidx1 * 32, &Raw1, 8);
    READ_MEMORY(BlockBase + Bidx2 * 32, &Raw2, 8);
    uint64_t B1 = SoftPshuflw(Rotl64(Raw1, 0), 0x00) ^ 0x0000063EEF1B0319ULL;
    uint64_t B2 = SoftPshuflw(Rotl64(Raw2, 0), 0x00) ^ 0x0000063EEF1B0319ULL;

    // FNV64 chain
    uint64_t Fv1 = 0x100000001B3ULL * Rotl64(B1, 40) + 0x01554577E835E9F4ULL;
    uint64_t Fv2 = 0x100000001B3ULL * Rotl64(Fv1, 41) + 0x01554577E835E9F4ULL;
    uint64_t RawPtr = B1 + (B2 ^ Fv2) + 2 * NameOff;

    // Pointer XOR chain
    uint64_t Step1 = Bswap64(RawPtr ^ 0x0000000043231D85ULL);
    uint64_t Step2 = Step1 ^ 0x80BE251500000000ULL;
    uint64_t EntryPtr = Bswap64(Step2 ^ 0x05A3065600000000ULL);
    return EntryPtr;
}

// ─── 6. String Decrypt (FNameEntry → readable string) ───────────────────────
// Header: length = hdr >> 5, isWide = (hdr & 0x20) != 0
// Key schedule: init = length + 0x0, advance = key * 0x0 + 0x0
...
static inline int FNameDecryptString(uint64_t EntryPtr, char* OutBuf, int MaxLen) {
    ...
    if (!IsWide) {
        ...
        uint32_t Key = (uint32_t)(Length + 0x0u);
        for (int I = 0; I < ByteLen; I += 2) {
            Buf[I]     ^= (uint8_t)(KeyTable[Key & 0x3Fu] >> 3);
            uint32_t Idx2 = (Key * 0x0u + 0x0u) & 0x3Fu;
            if (I + 1 < ByteLen)
...
constexpr uint32_t USkeleton_BoneArrayOffset = 0xE0;
...
namespace Offsets {
    ...
    constexpr uint32_t UStruct_SuperStruct    = 0xA8;
    constexpr uint32_t UStruct_ChildProperties = 0x100;
    constexpr uint32_t UStruct_PropertiesSize = 0x110;
    constexpr uint32_t FField_NamePrivate     = 0x70;
    constexpr uint32_t FField_Next            = 0x80;
    constexpr uint32_t FField_Owner           = 0xA0;
    constexpr uint32_t FField_ClassPrivate    = 0x90;
    constexpr uint32_t FProperty_Offset       = 0xC4;
    constexpr uint32_t FProperty_ElementSize  = 0xF8;
    constexpr uint32_t FProperty_ArrayDim     = 0xF0;
    constexpr uint32_t FStructProperty_Struct = 0x120;
    constexpr uint32_t FObjectProperty_Class  = 0x120;
    constexpr uint32_t FBoolProperty_FieldSize = 0x120;
    constexpr uint32_t FArrayProperty_Inner   = 0x128;
    constexpr uint32_t UFunction_NativeFunc   = 0x160;
} // namespace Offsets
