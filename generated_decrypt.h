...
// Generated by FrostDumper on Aug  9 2026 21:29:40
...
constexpr uint64_t GWORLD_RVA         = 0xE859548ULL;  // MODULE_BASE + this → UWorld**
...
// Decrypts the encrypted ComparisonIndex stored at FField + 0x60
// Pipeline: XOR64 → ROL16(0) → PSHUFB → ROL64(0)
...
static inline uint32_t DecryptFFieldName(uint64_t Encrypted) {
    ...
    { uint16_t W[4]; memcpy(W, &V, 8);
      for (int I = 0; I < 4; I++) W[I] = (W[I] << 0) | (W[I] >> 16);
      memcpy(&V, W, 8); }
...
static inline int32_t DecryptPropertyOffset(uint32_t Stored) {
    return (int32_t)(Bswap32(Stored) ^ 0x76C317A2u);
}
...
namespace Offsets {
    ...
    constexpr uint32_t UStruct_ChildProperties = 0xD0;
    constexpr uint32_t UStruct_PropertiesSize = 0xD8;
    constexpr uint32_t FField_NamePrivate     = 0x60;
    constexpr uint32_t FField_Next            = 0x80;
    constexpr uint32_t FField_Owner           = 0x78;
    constexpr uint32_t FField_ClassPrivate    = 0x90;
    constexpr uint32_t FProperty_Offset       = 0xB4;
    constexpr uint32_t FProperty_ElementSize  = 0x9C;
    ...
    constexpr uint32_t FBoolProperty_FieldSize = 0x108;
