...
// Generated by FrostDumper on Sep 10 2026 08:48:13
// Patch image size: 0x14091000
// =============================================================================
//
...
constexpr uint64_t GNAME_POOL_BASE    = 0x10A36B08ULL;  // MODULE_BASE + this
constexpr uint64_t KEY_TABLE_RVA      = 0x1095926CULL;  // MODULE_BASE + this
constexpr uint64_t GWORLD_RVA         = 0x10F2A590ULL;  // MODULE_BASE + this -> UWorld**
...
static inline int32_t DecryptPropertyOffset(uint32_t Stored) {
    return (int32_t)(Bswap32(Stored) ^ 0xC2CEEE92u);
}
...
// FNV64 fold: ROL(37) + ADD, ROL(48) + ADD
...
static inline uint64_t FNameResolveEntry(uint32_t CI) {
    ...
    // Shard hash
    uint64_t SeedAddr = ChunkAddr + 0x2490ULL;
    uint32_t SeedLo = 0, SeedHi = 0;
    READ_MEMORY(SeedAddr, &SeedLo, 4);
    READ_MEMORY(SeedAddr + 4, &SeedHi, 4);
    uint32_t H = 0x01000193u * Rotl32(SeedLo, 19) + 0x902D0766u;
    H = 0x01000193u * Rotl32(H, 13) + SeedHi + 0x902D0766u;
    H = 0x01000193u * Rotl32(H, 19) + 0x902D0766u;
    uint32_t T = Rotl32(H, 13);
    uint8_t Pa = (uint8_t)(-109 * T + 82);
    uint8_t Pb = (uint8_t)((0x01000193u * T + 0x902D0766u) >> 16);
    int Bidx1 = (Pa ^ Pb) & 7;
    int Bidx2 = (Bidx1 + 1) & 7;

    // Block decode
    uint64_t BlockBase = ChunkAddr + 0x24A0ULL;
    uint64_t Raw1 = 0, Raw2 = 0;
    READ_MEMORY(BlockBase + Bidx1 * 32, &Raw1, 8);
    READ_MEMORY(BlockBase + Bidx2 * 32, &Raw2, 8);
    uint64_t B1 = SoftPshuflw(Rotl64(Raw1, 0), 0x00) ^ 0x8785880100000000ULL;
    uint64_t B2 = SoftPshuflw(Rotl64(Raw2, 0), 0x00) ^ 0x8785880100000000ULL;

    // FNV64 chain
    uint64_t Fv1 = 0x100000001B3ULL * Rotl64(B1, 37) + 0x6292C37EFA7F5FA6ULL;
    uint64_t Fv2 = 0x100000001B3ULL * Rotl64(Fv1, 48) + 0x6292C37EFA7F5FA6ULL;
    uint64_t RawPtr = B1 + (B2 ^ Fv2) + 2 * NameOff;

    // Pointer XOR chain
    uint64_t Step1 = Bswap64(RawPtr ^ 0x000000005D4B82B8ULL);
    uint64_t Step2 = Step1 ^ 0x0000516500000000ULL;
    uint64_t EntryPtr = Bswap64(Step2 ^ 0xB8821A3800000000ULL);
    return EntryPtr;
}

// --- 6. String Decrypt (FNameEntry -> readable string) -----------------------
// Header: length = hdr >> 10, isWide = (hdr & 0x20) != 0
...
constexpr uint32_t USkeleton_BoneArrayOffset = 0xA0;
...
namespace Offsets {
    ...
    constexpr uint32_t UStruct_SuperStruct    = 0x128;
    constexpr uint32_t UStruct_ChildProperties = 0x128;
    constexpr uint32_t UStruct_PropertiesSize = 0xD0;
    constexpr uint32_t FField_NamePrivate     = 0x90;
    constexpr uint32_t FField_Next            = 0xE8;
    constexpr uint32_t FField_Owner           = 0x40;
    constexpr uint32_t FField_ClassPrivate    = 0x70;
    constexpr uint32_t FProperty_Offset       = 0xB0;
    constexpr uint32_t FProperty_ElementSize  = 0xB8;
    constexpr uint32_t FProperty_ArrayDim     = 0xBC;
    constexpr uint32_t FStructProperty_Struct = 0x100;
    constexpr uint32_t FObjectProperty_Class  = 0x100;
    constexpr uint32_t FBoolProperty_FieldSize = 0x118;
